...
\begin{proof}
	...
	\prob(J_1, \dots, J_n \in U) \leq 2^n (s/r)^{dn}.
	\end{equation}
	%
...
To complete the proof of Theorem \ref{mainTheorem}, we must show that $\hausdim(X) \geq (dn - \alpha)/(n - 1)$. In view of Definition \ref{defFrostmanItem}, we will do this by constructing a Frostman measure of appropriate dimension supported on $X$. We start by recursively defining a positive function $\mu: \bigcup_{i = 1}^\infty \B^d_{l_i} \to [0,1]$. Set $\mu([0,1]^d) = 1$, and $\mu(I) = 0$ for every other $I \in \B^d_{l_0}$. Suppose now that $\mu(I)$ has been defined for all cubes $I \in \B^d_{l_{k-1}}$, and let $I \in \B^d_{l_k}$. Consider the unique `parent cube' $I^* \in \B^d_{l_{k-1}}$ for which $I \subset I^*$. Define
%
\begin{equation} \label{muRecurse}
	\mu(I) = \begin{cases} {\mu(I^*)}/{\# \B^d_{l_k}(X_k \cap I^*)} & \textrm{if}\ I \subset X_k,\\
0 & \textrm{otherwise}. \end{cases}
\end{equation}
%
Observe that $\mu(I) > 0$ if and only if $I \cap X \neq \emptyset$, and for each index $k \geq 1$, if $I^* \in \B_{l_{k-1}}^d$, 
%
\begin{equation}\label{muBreakDown}
	\sum \{ \mu(I) \setcolon I \in \B_{l_k}^d(I^*) \} = \sum \{ \mu(I) \setcolon I \in \B_{l_k}^d(X_k\cap I^*) \} = \mu(I^*).
\end{equation}
%
In particular, for each index $k$ we have
%
\[ \sum_{I \in \B_{l_k}^d} \mu(I)=1. \]
%
By a standard argument \cite[Proposition 1.7]{Falconer}, $\mu$ extends to a Borel probability measure $\mu$ supported on $\bigcap X_k = X$. To complete the proof of Theorem \ref{mainTheorem} we will show that $\mu$ is a Frostman measure of dimension $(dn - \alpha)/(n - 1)-\epsilon$ for every $\epsilon>0$. 
